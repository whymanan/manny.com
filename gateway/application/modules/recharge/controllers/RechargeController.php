...
//new
  ...
  {
      $query="Select `member_id` FROM `user` where `role_id`!=94";
      $sql = $this->db->query($query);
      $result = $sql->result_array();
      return $result;
  }
...
//   $writer->save("php://output");
       ...
       if($transition[0]['transection_amount']==$response1->data->amount && $transition[0]['transection_mobile']==$response1->data->canumber)
       {
        if($response1->response_code==1 && $response1->status==1 && $response1->data->status==1)//success
        {
                if($this->db->where('id',$status[0]['id'])->delete('status_api'))
                {
                  ...
                  self::commition_distribute_biil_status($user[0]['user_id'],$transition[0]['service_id'],$response1->data->amount,$response1->data->operatorname,$transition[0]['transection_id'],$transition[0]['api_requist']);   
  ...
  public function transection_service_status_bill($refid)
  {
    ...
    //bill comission
  public function commition_distribute_biil_status($id, $service, $transection, $operaor,$transection_id,$api_requist)
  {
