  ...
  if ($_POST) 
  {
    ...
    // photo
      ...
      //  phto
          ...
          // pan
          // din
          $count5 = count($_FILES['din']['name']);
          for($i=0; $i<$count4; $i++)
            {
      
              $_FILES['din']['name']= time().$files['din']['name'][$i];
            $_FILES['din']['type']= $files['din']['type'][$i];
            $_FILES['din']['tmp_name']= $files['din']['tmp_name'][$i];
            $_FILES['din']['error']= $files['din']['error'][$i];
            $_FILES['din']['size']= $files['din']['size'][$i];
            $config['upload_path'] = 'uploads/gstfile/';
            // $config['allowed_types'] = 'jpg|png|jpeg';
            $config['max_size'] = '2000000';
            $config['remove_spaces'] = true;
            ...
            $images5[] = $_FILES['din']['name'];
            }
            $fileName5 = implode(',',$images5);
            $filename5 = explode(',',$fileName5);
       // din end
      $name=$this->input->post('name'); 
      $m_name=$this->input->post('father_name');
      ...
      $query2= $this->common_model->insert($regdata, 'gst_registration');
      if($query2){
        echo '100';
      }else{
        echo '23';
