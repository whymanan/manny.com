...
if(!function_exists('randomPassword')){
 ...
 if(!function_exists('callAPI')) {
    	...
    	if (!function_exists('transaction_check')) {
    	    function transaction_check($rrn) {
    	        
    			$ci =& get_instance();
                $ci->load->database();
                     
                $query = $ci->db->get_where('submit_transection',array('reference_number'=>$rrn ));
                if($query->num_rows() == 1){
                    
                    $result = $query->row();
                    if($result->transection_type == 'CW' && $result->transection_status != 1 || $result->transection_respcode != 1){
                    ...
                    $result = [
                          "Status" => 403 ,
                          "Msg" => "Transaction Not Found Contact Admin"
                          ];
