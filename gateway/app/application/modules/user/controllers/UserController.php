...
'user_status' => $status
        ...
        $this->db->where('user_id', $user_id);
       $query=  $this->db->update('user', $filedata);
      redirect('users', 'refresh');
} else {
    // echo 'Invalid password.';
   redirect('users', 'refresh');
...
public function get_users()
    {
        ...
        if (!empty($uri)) {
          ...
          Join roles on roles.roles_id=user.role_id WHERE user.parent = '{$this->session->userdata('user_id')}'";
        ...
        $query .= " AND user.kyc_status = 'new' ";
          ...
          $sub_array[] = '<a href="' . base_url('users/edit?q=') . $row['user_id'] . '"> <button type="button" class="btn btn-sm btn-secondary"  data-placement="bottom" title="Edit Menu Information"><i class="fa fa-pencil-alt"></i></button></a>
             ...
             <button type="button" class="btn btn-sm btn-primary"  data-placement="bottom" onclick="showModal(' . $row['user_id'] . ')" title="Delete Menu Information"><i class="fa fa-trash-alt"></i></button>';

                $sub_array[] = '<button type="button" class="btn btn-sm btn-primary"  data-placement="bottom" onclick="showModals(' . $row['user_id'] . ')" title="Block User"><i class="fa fa-ban"></i></button> ';
                $sub_array[] = $row['member_id'];
                $sub_array[] = $row['parent1'];
