...
public function __construct()
    ...
    public function earning_by_date($member_id,$date){
        $query = $this->db->where(['transection_status'=> 1,'member_id'=> $member_id])
                            ->like('created',$date)
...
// pre( $this->db->last_query());exit;
  ...
  public function getBeneficiaryMobile($bid) {
     ...
     function create($data) {
      $this->db->insert_batch('diectors', $data);
  }
  ...
  function find_member($table,$where,$id){
    ...
    // 
    function fetch_single_details($uid)
	{
		$this->db->where('reg_id', $uid);
		$data = $this->db->get('gst_registration');
		$output = '<table width="100%"  style="border: 1px solid black;margin">';
      ...
      <td>
               <td style="height:5%;width:50%;border: 1px solid black;">Java</td>
            <td style="5%:100px;width:50%;border: 1px solid black;">50</td>
</td>
  </tr>
  <tr>
  ...
  <tr>
    ...
    <td>Italy</td>
	 ...
	 function fetch_invoice_details($uid)
	{   
		$this->db->where('reg_id', $uid);
		$query = $this->db->get('gst_registration');
		return $query;

	}
	 function fetch_u_details($user_id)
	{  
	...
	$this->db->where('fk_user_id', $user_id);
		...
		return $query;	
		 ...
		 function fetch_aeps_bill_byid($id)
	{  
	    
	$this->db->where('primary_id', $id);
		$query = $this->db->get('submit_transection');
		return $query;	

	}
	 function fetch_user_details_byid($user_id)
	{  
